...
public class People {
	...
	public static Emojicon[] getEmojiGroup(int start , int end) {
		List<Emojicon> emojiList = new ArrayList<Emojicon>();
		for (int i = start; i < end; i++) {
			if(end>=DATA.length) break ;
			emojiList.add(DATA[i]);
		}
		Emojicon deleteEmoj = Emojicon.fromCodePoint(0xddddd);
		deleteEmoj.setDelete(true);
		emojiList.add(deleteEmoj);
		Emojicon[] result = new Emojicon[emojiList.size()];
		emojiList.toArray(result);
		return result;
	}
