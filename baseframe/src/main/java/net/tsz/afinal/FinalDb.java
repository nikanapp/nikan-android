...
public class FinalDb {
	 ...
	 * @return
	 */
	public <T> List<T> find(Class<T> clazz, int start , int offset) {
		checkTableExist(clazz);
		return findAllBySql(clazz, SqlBuilder.getSelectSQL(clazz)
				+ " LIMIT " + start+" , "+offset);
	}
	 ...
	 * @return
	 */
	public <T> List<T> find(Class<T> clazz,String orderBy ,  int start , int offset) {
		checkTableExist(clazz);
	 ...
	 * @return
	 */
	public <T> List<T> findWhere(Class<T> clazz,String where ,String orderBy ,  int start , int offset) {
		checkTableExist(clazz);
		return findAllBySql(clazz, SqlBuilder.getSelectSQL(clazz) + " WHERE "+where+" ORDER BY " + orderBy
				+ " LIMIT " + start+" , "+offset);
	}
	...
	public static class DaoConfig {
		...
		private int dbVersion = 2; // 数据库版本
		private boolean debug = false; // 是否是调试模式（调试模式 增删改查的时候显示SQL语句）
	 ...
	 * @param clazz
	 */
	public int getLastSaveId(Class<?> clz) {
		String sql =  "select last_insert_rowid() from "+TableInfo.get(clz).getTableName();
	...
	public int count(Class<?> clz , String where){
		checkTableExist(clz);
		String sql =  " select count(*) from "+TableInfo.get(clz).getTableName();
	...
	public String getTable(Class<?> clz){
		return TableInfo.get(clz).getTableName();
	}
}
