...
public class GpsCoordinateUtils {
    ...
    public static boolean isOutOfChina(double latitude, double longitude, boolean precision) {
        if (precision) {
            return noneMatchPointInPolygon(latitude, longitude);
        } else {
    ...
    private static boolean noneMatchPointInPolygon(double latitude, double longitude) {
        for (Point[] point:CHINA_POLYGON) {
            if (pointInPolygon(point, latitude, longitude)) {
                return false;
            }
        }
        return true;
