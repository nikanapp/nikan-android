...
public class JazzyViewPager extends ViewPager {
	...
	private static final float ALPHA_ZOOM_MAX = 0.0f;
	...
	public JazzyViewPager(Context context, AttributeSet attrs) {
		...
		setFadeEnabled(ta.getBoolean(R.styleable.JazzyViewPager_jazzy_fadeEnabled, false));
...
//}
	...
	private void animateAlphaZoom(View left, View right, float positionOffset, boolean in) {
		if (mState != State.IDLE) {
			float trans = UiUtils.dip2px(getContext(), 130);
			if (left != null) {
				manageLayer(left, true);
	...
	public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
		Log.d(TAG, " position = "+position+" ,   positioniffset= "+positionOffset+"  positionOffsetPixels= "+positionOffsetPixels);
...
//		mRight = getChildAt(position+1);
		...
		case AlphaZoomIn:
			animateAlphaZoom(mLeft, mRight, effectOffset, true);
			break;
		}
